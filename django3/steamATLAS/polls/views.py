...
def tag_results(request):
    ...
    for eachID in recommended_games:
        ...
        for row in tester:
            ...
            print(row[1].encode('ascii', 'ignore'))
            print(row[2].encode('ascii', 'ignore') )
            print(str(row[3]).encode('ascii', 'ignore') )
            print(row[4].encode('ascii', 'ignore') )
            print(str(eachID).encode('ascii', 'ignore'))
            '''
    ...
    return HttpResponseRedirect(reverse('polls:game_recommended', kwargs = {'tag' : username}))
